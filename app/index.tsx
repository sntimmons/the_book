...
const DEV_NAV: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Reels Feed', route: '/(tabs)/reels' },
  ...
  },
  {
    ...
    items: [
      { label: 'Dashboard Home', route: '/(tabs)/business' },
      { label: 'Bookings', route: '/(tabs)/business/bookings' },
      { label: 'Availability', route: '/(tabs)/business/availability' },
      { label: 'My Clients', route: '/(tabs)/business/clients' },
      { label: 'Analytics', route: '/(tabs)/business/analytics' },
      { label: 'Portfolio', route: '/(tabs)/business/portfolio' },
      { label: 'Posts & Reels', route: '/(tabs)/business/posts' },
      { label: 'Payouts', route: '/(tabs)/business/payouts' },
      { label: 'Services / Pricing', route: '/(tabs)/business/services' },
      { label: 'Settings', route: '/(tabs)/business/settings' },
    ],
  },
  {
...
const SITE_MAP: SiteMapRoute[] = [
  ...
  { label: '/(tabs)/business/clients', route: '/(tabs)/business/clients', status: 'Stub' },
  { label: '/(tabs)/business/analytics', route: '/(tabs)/business/analytics', status: 'Stub' },
  { label: '/(tabs)/business/portfolio', route: '/(tabs)/business/portfolio', status: 'Stub' },
  { label: '/(tabs)/business/posts', route: '/(tabs)/business/posts', status: 'Stub' },
  { label: '/(tabs)/business/payouts', route: '/(tabs)/business/payouts', status: 'Stub' },
  { label: '/(tabs)/business/services', route: '/(tabs)/business/services', status: 'Stub' },
  { label: '/(tabs)/business/settings', route: '/(tabs)/business/settings', status: 'Stub' },
...
export default function WelcomeScreen() {
  ...
  // DEV-ONLY: real sign-in as the selected account so a real session (and
  ...
  async function signInAs(account: DevAccount) {
    ...
    Alert.alert('Signed in as', `${account.label}\n${data.session.user.email}`)
  }

  // DEV-ONLY: the email of the real session currently signed in, if any.
  function activeSignedInEmail(): string | null {
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.devMenuSheet, { marginTop: insets.top }]}>
            ...
            >
              {/* DEV-ONLY: Act as account via real sign-in (kept at the top) */}
              <Text style={styles.devSectionLabel}>ACT AS ACCOUNT (DEV)</Text>
              <View style={styles.impersonateActiveRow}>
                <Feather
                  name={activeSignedInEmail() ? 'user-check' : 'user'}
                  size={13}
                  color={activeSignedInEmail() ? '#4CAF50' : 'rgba(240,232,213,0.3)'}
                />
                <Text style={styles.devNavLabel}>
                  {activeSignedInEmail()
                    ? `Signed in: ${activeSignedInEmail()}`
                    : 'Signed out (no session)'}
                </Text>
              </View>
              {DEV_ACCOUNTS.map((account) => {
                const isActive = !!user?.email && user.email === account.email
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  await signOut()
                  Alert.alert('Signed out', 'No account is signed in.')
                }}
              >
                <Feather name="log-out" size={13} color="#C8922A" />
