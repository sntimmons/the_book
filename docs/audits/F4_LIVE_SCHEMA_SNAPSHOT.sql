-- =============================================================================
-- F4 LIVE SCHEMA SNAPSHOT  (public schema)  -- READ-ONLY CAPTURE, NOT FOR EXECUTION
-- =============================================================================
-- Source: live Supabase project kxregomuawwcqvisuhtr (Postgres 17.6), captured
-- read-only via pg_get_* catalog functions through the Management API SQL path.
-- This file is a REFERENCE ARTIFACT for building a canonical baseline later.
...
-- ============================= 1. EXTENSIONS =============================
-- App-relevant: pgcrypto (gen_random_uuid), uuid-ossp. Supabase-managed:
-- supabase_vault, pg_stat_statements. plpgsql is a default. A canonical app
-- baseline should NOT recreate Supabase-managed infra (see plan doc).
create extension if not exists "pg_stat_statements" with schema extensions;  -- v1.11  -- supabase-managed / infra
create extension if not exists "pgcrypto" with schema extensions;  -- v1.3  -- app-relevant
...
-- ============================= 11. STORAGE =============================
-- Buckets (config rows in storage.buckets):
--   contract-pdfs: public=False size_limit=None mime=[None]
--   contract-signatures: public=False size_limit=None mime=[None]
--   posts-media: public=True size_limit=52428800 mime=[image/jpeg,image/png,image/webp,image/gif,video/mp4,video/quicktime,video/webm]
--   provider-media: public=True size_limit=52428800 mime=[image/*,video/*,mp4/*]
-- storage.objects policies (reconstructed; SECURITY findings preserved, NOT fixed):
