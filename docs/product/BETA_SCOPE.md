...
**Current implementation:** `identity_verified` state exists; providers cannot self-set it (owner-immutable; admin/service-role only). A complete ID-verification integration is **not built**. No client- or provider-facing verification initiation flow exists. Transactions are **not** yet gated on verification. So current UX is **PARTIAL** — do not imply a complete automated ID-verification process exists today.
