...
- **Booking conversations are unaffected** and remain open.
- Enforced **server-side** (not just UI): migration `20260901000000` adds `conversation.request_status`/`request_opened_at` and triggers that allow only the one initial client message while pending, block the provider from messaging while pending, block messages after decline, and restrict accept/decline to the provider (client cannot accept/decline; client can re-open a declined request). The `clients_public`/participant RLS is unchanged. Structured notifications/moderation are **not** built.
- **Open (future):** if a pre-booking conversation later leads to a booking, conversation-merging is not specially handled beyond attaching `booking_id` (which opens the conversation); no cooldown/anti-spam timing is decided.
