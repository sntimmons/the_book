...
- **Resolution.** **The latest SERVICE received.** The contributing review is the one tied to the most recently completed eligible service, ordered on `bookings.completed_at`; review submission time only breaks a tie between services completed in the same transaction, and review `id` is the final total order. The reasoning is that PD-091's promise is about the most recent RELATIONSHIP, and a late review of an old service replacing a newer one breaks that promise for no gain — and that `completed_at` is already the anchor for eligibility, the window and reveal, so one chronology governs the whole system instead of two that can disagree. `20261083000000`.
- **Status:** CLOSED — resolved by **PD-092**, 2026-09-11
...
- **Resolution.** **No manual or `service_role` rating pin, of any kind.** Two things came out of closing this. First, PD-092 largely dissolved the filed question: the ordering key is now `bookings.completed_at`, so a supplied `created_at` only breaks ties between services completed in the same transaction. Second — and this is what the question was actually worth — **a much larger pin was found next to it that nobody had filed**: `public.providers.rating`, a second numeric column no recompute has ever written, granted to `anon`/`authenticated`, published by both public provider views, and used as the **ranking and min-rating filter key for provider search** while every display surface read `average_rating`. Every row held 0, so nothing looked wrong, and any value written there would have been a permanent, invisible, hand-set marketplace position. `rating` is now a derived mirror, and an invariant refuses to STORE any reputation value the canonical computation does not produce — with no role carve-out, including `service_role`. `20261084000000`.
- **Status:** CLOSED — resolved by **PD-094**, 2026-09-11
