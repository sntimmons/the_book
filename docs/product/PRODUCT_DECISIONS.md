...
- **Evidence:** Founder ruling, 2026-09-04. Implemented by `supabase/migrations/20260909000000_barter_interest_release.sql` (`release_barter_interest`). **Not yet reachable in the app** — the server capability landed without a UI control, so the stranded-slot defect is still live for users until that control ships. `releaseInterest` in `lib/barter.ts` is the reserved client seam for that control and is intentionally uncalled; it must not be removed as dead code. Shipping the control also requires a `release` operation in `lib/barterErrors.ts`, since the RPC raises both `insufficient_privilege` and `check_violation` and neither has copy today.
- **Status:** Locked
