...
- **Evidence:** Founder ruling, 2026-09-04. Implemented by `supabase/migrations/20260909000000_barter_interest_release.sql` (`release_barter_interest`). **Not yet reachable in the app** — the server capability landed without a UI control, so the stranded-slot defect is still live for users until that control ships.
- **Status:** Locked
