...
- **Evidence:** Founder ruling, 2026-09-05. Implemented by `supabase/migrations/20260927000000_barter_agreement_finalization.sql` and `20260928000000` (a field-reference bug in the post-agreement guard, caught by B5B on first run after apply). Asserted in `supabase/tests/agreement.test.sql` — zero/one/old-version acceptances refused, stranger refused, participant may finalize, idempotent, one per negotiation/post, accepted-version reference immutable, post closes atomically and cannot reopen, counter and release refused after agreement, released negotiation cannot finalize, every new object's grants/RLS/definer posture pinned — and, for the races B5B cannot stage, `scripts/negotiation-concurrency.mjs` (finalize × finalize, finalize vs counter, finalize vs release, each with overlap proven). Client states in `lib/negotiationState.ts` (`agreed` = "Ready to confirm trade", `confirmed` = "Trade confirmed") and `lib/tradeActivity.ts` (a Confirmed trades section), tested.
- **Status:** Locked
