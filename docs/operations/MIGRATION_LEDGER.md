...
| `public.my_barter_obligations` / `public.my_trade_activity` (VIEWS, not functions) | `20261011000000` / `20260929000000` | **THE TWO NOW DIFFER — read the right one.** `my_barter_obligations`: **`20261039000000_barter_review_request.sql`**. `my_trade_activity`: **`20261027000000_suppression_computed_once.sql`**. | Listed here although this table is named for functions, because both views are recreated IN FULL by `create or replace view` and the same copy-forward hazard applies. `20261012000000` is the more discoverable file — it carries the design rationale — and copying its `my_trade_activity` body forward would reinstate the inline eligibility predicate `20261013000000` removed, while copying its `my_barter_obligations` body would drop `can_report_no_show` and `no_show_reason`. **Both were redefined again for adjudication:** `20261020000000` feeds `cancelled OR adjudicated` into the three derived functions and appends `terminal_outcome` / `adjudicated_at` (written from `20261016000000`); `20261021000000` appends `my_terminal_outcome` / `their_terminal_outcome` (written from `20261013000000`). **Neither view exposes `rationale` or `adjudicator_user_id`, and neither may** — PD-067 makes those internal, and a view is exactly how that would be undone by accident. **`20261027000000` was the live definition of BOTH**, taken from `pg_get_viewdef` rather than from any migration: it computes the suppression predicate ONCE in a lateral instead of inlining it three times, and it had to DROP both views (not `create or replace`) because renaming a function parameter requires dropping the function. Two consequences a future editor must know: the obligation view is **no longer auto-updatable** (it now joins), so writes are refused with `55000` by the rewriter before the grant check returns `42501` — both refusals hold, and B5B pins the property rather than the code; and `security_invoker=true` on **both** views is now pinned by B5B, because recreating either without it is a silent, total RLS read bypass. **SUPERSEDED FOR ONE VIEW ONLY, 2026-09-09:** Correction 3's `20261039000000` recreated **`my_barter_obligations`** to add PD-072's deliverer review request as a THIRD route into Under Review, and left `my_trade_activity` alone. This row went on naming `20261027000000` for both — **the exact failure this table exists to prevent**, found by the post-Session-8 state reconciliation rather than by anyone editing the view. Copying `20261027000000`'s obligation body forward would silently delete the third route, and a deliverer whose counterparty stopped opening the app would lose the only move they have. One function, one row, one answer — a row that answers for two objects is a row that will be wrong about one of them. |
...
| `public.enforce_no_change_after_agreement` | `20260927000000_barter_agreement_finalization.sql` | **`20260930000000_confirmed_trade_sqlstate.sql`** | Referenced `new.version_id` inside a CASE branch meant for `barter_version_acceptances`; PL/pgSQL resolves NEW's fields regardless of branch, so on a `barter_proposal_versions` row it raised 42703 and blocked EVERY version insert. Caught by B5B on the first run after apply. Now reads the row through `to_jsonb(new)`. **STALE ROW, CORRECTED 2026-09-10 — the THIRD of this class, and the first found MECHANICALLY rather than by someone happening to look.** This cell named `20260928000000` while TWO later migrations had redefined the function: `20260929000000` made it **fail closed** when the proposal id cannot be resolved (raising `internal_error` instead of returning NEW, so an unresolvable write is refused rather than waved through), and `20260930000000` gave the confirmed-trade refusal its own **`PT409`** so client copy can distinguish *this trade is confirmed* from *this negotiation is in the wrong state*. A copy-forward from `20260928000000` would have deleted BOTH — silently reopening post-agreement writes whenever the proposal lookup failed, which is the fail-open direction. Found by sweeping every `create ... function|view` in the chain for its newest defining migration and comparing against this table; see the safeguard task in [ROADMAP.md](../product/ROADMAP.md). |
...
| `public.recompute_provider_rating_for` | `20260902000000_reviews_phase0_foundation.sql` | **`20261081000000_the_weakest_lock_that_works.sql`** | Redefined THREE times. `20261077000000` made `average_rating` the mean of the **latest revealed review per distinct client** (PD-091) and added `rating_client_count`. `20261080000000` then found the counting ran on the statement snapshot while the row lock was only taken by the UPDATE at the end, so two concurrent reviews each wrote a total omitting the other — a silent lost update. `20261081000000` fixed the fix: `for update` is the **only** lock mode that conflicts with the `for key share` that `provider_reviews_provider_id_fkey` already takes on the same row, so two honest simultaneous reviewers could deadlock (`40P01`), and the review screen handles only `23505`. It is now `for no key update` — conflicts with itself, so recomputers still serialize; does not conflict with the FK. **This is exactly the class `20261014000000` documented** (*"enumerate the IMPLICIT locks too"*) and it was not re-applied here. Three properties are pinned against `pg_proc.prosrc` in `supabase/tests/reviews_phase2.test.sql` § 6b — lock present, lock BEFORE the counting, and lock not the bare `for update`. |
| `public.enforce_review_created_at_is_the_server` (new) / `public.recompute_rating_on_review_hold` (new) | `20261079000000_the_latest_is_the_servers_to_decide.sql` | **`20261081000000`** (trigger `WHEN` clause + comment) | `provider_reviews.created_at` was `DEFAULT now()` with INSERT granted on every column and no stamping trigger — and PD-091 had just made it decide WHICH of a repeat client's reviews forms the public rating. A client could pin their own voice permanently, uncorrectably, since reviews can never be edited or deleted. Same class as `contract_signatures.signed_at` in `20261073000000`, one field over. The stamping trigger is named `a_` so it sorts **first**, ahead of anything that reads the value. `recompute_rating_on_review_hold` makes a dispute move the STORED aggregate, not just hide the row; `20261081000000` gave it a `WHEN` clause so it stops costing a PL/pgSQL call on every booking UPDATE. **Carve-out constraint recorded in the function comment:** any future server-side review writer inherits the power to pin a rating permanently (OQ-079). |
| `public.provider_review_revealed` / `public.client_review_revealed` | `20260902000000_reviews_phase0_foundation.sql` | **`20261082000000_a_dispute_is_not_a_rating_control.sql`** | **Reveal now LATCHES.** Phase 0 held reveal outright while `under_review` was true, and `20261079000000` made the stored rating drop on the hold to match. Product reversed both (**PD-093**): filing a dispute is an act by a participant with no adjudication behind it, and a rating that falls when someone complains is an unreviewed veto over the other side's public record — the provider who dislikes a 1-star and the client who wants leverage reach for the same button. A review **not yet revealed** when the hold opens still stays held; a review **already revealed** stays revealed and keeps counting. Derived, not stored (**PD-070**): reveal is partly TIME-based, so a `revealed_at` stamp would be wrong whenever nobody happened to be writing. While a hold is open, the ordinary rule is evaluated as of the new `bookings.under_review_at`. **That column has NO `service_role` carve-out** — against the local convention for timestamp stamps, and deliberately: it decides which already-public reviews a dispute suppresses, and `under_review` is already service_role-only, so a carve-out would hand the only role that can open a hold the choice of its retroactive effect. Pre-existing holds are anchored at `completed_at`, which makes the latch false for all of them — **nothing is published retroactively**. **CORRECTED BY `20261086000000`:** as shipped, that backfill wrote NOTHING — a plain UPDATE run ten lines after the stamping trigger was created, which did not change `under_review`, so the trigger took its unchanged-hold branch and wrote `old.under_review_at` (NULL) straight back. No error, and both this row and the migration's own comment asserted a post-condition that never held. The OUTCOME was unaffected, because the latch tests for null and fails closed — but the fail-closed guard was doing the load-bearing work while the documentation credited the anchor, which is how a guard comes to be removed. `20261086000000` heals a legacy hold INSIDE the function (never from a supplied value — that would reopen exactly what PD-093 closed), and `reviews_phase2.test.sql` § 5e now pins the whole-table post-condition: `bookings where under_review and under_review_at is null` = 0. |
| `public.stamp_under_review_at` (new) → trigger `f_bookings_under_review_at_server` | `20261082000000` | **`20261085000000_the_block_gate_stays_last.sql`** | The trigger shipped as `zzz_bookings_under_review_at_server`, which sorted past `zz_bookings_submit_not_blocked` — the Session 8 block gate that `safety_operator.test.sql` § 14a pins as **literally last on `bookings`**, read off the live catalog because `20261055000000` had already shipped gates that a comment claimed fired last and did not. Nothing was unsafe (a stamp cannot influence whether a submit is blocked), but the fix is to stop sorting past a tripwire there is no need to sort past. `f_` is the whole requirement and nothing more: **after** `enforce_booking_write_integrity`, which forces `under_review := false` for non-service callers and whose result the stamp must observe. **Renaming this trigger changes behaviour.** |
| `public.provider_reputation_canonical` (new) | `20261083000000_the_latest_service_not_the_latest_receipt.sql` | **`20261083000000`** | **THE single definition of the reputation rule** (PD-091 + **PD-092**). It exists because the rule was written TWICE — in `recompute_provider_rating_for` and `provider_reputation` — with the ordering clause copied between them, and PD-092 added a JOIN to that clause: exactly the edit that lands in one copy. PD-092 changes "latest" from the latest review WRITTEN to the latest SERVICE completed (`bookings.completed_at desc`), with the review's server-stamped `created_at` and then `pr.id` as the documented tie-break — `completed_at` is `now()`, so a provider completing two of one client's bookings in ONE transaction ties them, and `distinct on` without a total order returns an implementation-defined row. **Both callers delegate here and `reviews_phase2.test.sql` § 6b asserts that EXACTLY ONE function in the schema contains the rule** — do not restate the query anywhere. |
| `public.providers.rating` / `public.reputation_is_derived` (new) | `20260829000000_canonical_live_baseline.sql` | **`20261084000000_a_rating_nobody_can_pin.sql`** | **`providers.rating` was a live manual rating pin that nobody had filed.** A second numeric column that **no recompute has ever written**, yet SELECT-granted to `anon`/`authenticated`, published by both public provider views, and used by `hooks/useProviders.ts` as the **ranking and min-rating filter key for provider search** while every display surface read `average_rating`. Every row held 0, so nothing looked wrong; the moment reviews landed, search would rank everyone on a stale zero, and any value written there would be a permanent, invisible, hand-set marketplace position. It is now a **derived mirror** of `average_rating`, and `reputation_is_derived` refuses to STORE any of the four reputation columns at a value `provider_reputation_canonical()` does not produce. **No role carve-out, including `service_role`** (**PD-094**, closing OQ-079): the ruling is about what may be STORED, not who is writing, so there is no carve-out to find. A raw restore is corrected by the next recompute rather than preserved — which is correct for derived data, since it is reproducible by definition. Search now orders and filters on `average_rating`. |
| `public.revealed_provider_review_ids` (new) | `20261087000000_the_public_list_is_the_public_list.sql` | **`20261087000000`** | **The read policy answers a different question than a public list asks.** `provider_reviews_read` is `auth.uid() = reviewer_user_id OR revealed` — correct as a privacy boundary, because a reviewer must be able to read back what they wrote. `fetchRevealedProviderReviews()` used it as the definition of the PUBLIC list, so the author, inside their own blind window, opened the provider's profile from the screen that had just promised their review stays private and found it listed and counted. Narrowing the POLICY would break the author's legitimate read, which the blind window depends on — *may I read this row* and *is this row public* are genuinely different questions and only the second belongs to a public list. Discloses nothing new: every id it returns is already readable by `anon` through the same predicate. The app filter FAILS CLOSED (no list rather than a possibly-blind one), and deliberately does not recompute reveal client-side — an earlier client-side re-filter was removed because it rebuilt reveal from `bookings`/`client_reviews` reads that RLS blocks for non-participants, silently dropping real reviews for exactly the readers the list is for. |
| `public.community_posts` (actor + intent model) / `public.community_posts_visible` | `20260829000000_canonical_live_baseline.sql` | **`20261088000000_community_is_for_people_who_need_a_service.sql`** | **Community was provider-only by TABLE SHAPE, not by policy choice**: `provider_id` was `NOT NULL REFERENCES providers(id)`, so no client could ever be represented. `provider_id` is now nullable and paired with an EXPLICIT `author_kind` — not inferred from the FK, because the two agree today and drift the first time someone who owns a business posts as a person. A `provider` post is rewritten server-side to `caller_eligible_provider_id()`, which is also the gate that stops a **deapproved** provider posting (the rule barter writes took in `20261048000000` and Community never did). `intent` is CHECK-constrained and PAIRED with `author_kind`, so a client cannot post a provider type and vice versa — deliberately unlike the legacy `category` column, which is free text and whose lookup maps anything unrecognised to "Other". `expires_at` exists only for `open_today` and MUST exist for it, so "a permanent post that says open today forever" is unrepresentable. The VIEW carries three rules the base table cannot: the PD-089 block filter, the expiry, and the requirement that an open_today note's provider is STILL in `providers_open_today()`. **`is_active` is documented as RESERVED AND UNUSED** — it reads like a take-down mechanism, nothing writes it, and the only UPDATE policy is the author's own (OQ-082). |
| `public.enforce_community_post_integrity` (new) | `20261088000000` | **`20261091000000_two_refusals_and_a_clearer_no.sql`** | Redefined twice, both times for something the first version got wrong. **`20261090000000`**: the UPDATE-immutability branch ran BEFORE the service_role carve-out, and `tagged_provider_id` is `ON DELETE SET NULL` — a set-null referential action is an UPDATE on this table, so **deleting a provider who had ever been recommended would have failed outright**, the same shape as the two OQ-077 erasure defects. The carve-out moved first; this is explicitly NOT the `20261082000000` case, where the no-carve-out was correct because the field decided what a participant could suppress. **`20261091000000`**: two refusals arrived with the wrong REASON — a client posting `open_today` was told to publish hours they cannot have, and a shoutout naming nobody was told "that provider cannot be recommended" about a provider never named. Both structural facts are now checked FIRST, for the message; the CHECK constraints remain, because a message is not a guarantee. |
| `public.community_replies` / `public.update_community_like_count` / `public.update_community_reply_count` | `20260829000000_canonical_live_baseline.sql` | **`20261089000000_an_answer_is_attached_to_the_question.sql`** | **THE COUNTERS HAD NEVER WORKED.** Both trigger functions were SECURITY INVOKER — running as the liker/replier — and updated `community_posts`, whose only UPDATE policy is `auth.uid() = user_id`. Every like or reply on someone ELSE's post, which is the entire point, updated **zero rows silently**; the app's optimistic local increments hid it until reload. Now SECURITY DEFINER with `search_path` pinned and `greatest(…, 0)`, **with no UPDATE policy widened** — the author's edit boundary is untouched. Both counts were re-derived. Replies gained `author_kind` and `kind` (`can_help` is a provider offering the work, providers only), a DEFINER `_visible` view, and a **block gate on the WRITE** — the read filter hid the post, but a blocked party holding a post id could still reply under it, and replies are where contact actually happens. Like/bookmark policies are restated as own-row-only in every direction: the like READ was provider-gated, so a client could insert a like and never read it back. |
| `public.providers_open_today` (unchanged) → Community's Open Today | `20261044000000_open_today_is_a_function_not_a_column.sql` | **`20261044000000`** (do not redefine) | **PD-096 builds ON this function rather than storing a second answer.** An Open Today note is refused unless the provider is already in this set (`PT430`), and the view re-checks membership on every read — so blocking the date stops the note surfacing immediately, before its expiry. **Do not add a "posted open today" flag anywhere**: the availability tables are the truth about whether a provider is open, and a second source of truth is one that can contradict the first. It still means "open today", NOT "has a free slot". |
| `public.community_posts.tagged_provider_id` FK / `public.provider_availability.timezone` | `20261088000000` / `20260829000000` | **`20261092000000_the_half_of_the_erasure_fix_i_missed.sql`** / **`20261093000000_one_bad_timezone_must_not_close_the_feed.sql`** | **TWO FINDINGS FROM THE SECURITY REVIEW OF THIS BRANCH, both of which the branch itself created.** (a) `20261090000000` was written to stop a recommended provider blocking their own erasure — `ON DELETE SET NULL` on `tagged_provider_id` is an UPDATE that fired the integrity trigger. It moved the trigger's carve-out and **left the CHECK**, which fails on the same set-null with `23514`; a CHECK has no carve-out to move. So the defect survived the fix for it, **and the ledger recorded it as closed**. Worse, it was GRIEFABLE: any client could make any approved provider permanently undeletable with one recommendation. Now `ON DELETE CASCADE` — a shoutout's whole subject is the provider it names, and the CHECK (PD-097) says it must name someone, so a shoutout with nobody in it is not a degraded recommendation. This does NOT pre-empt OQ-077: a public recommendation is not transaction evidence and is already author-deletable. (b) `providers_open_today()` computes `now() at time zone <provider-written text>` over every provider's availability, and this branch put it in the WHERE clause of the community feed view — so **one provider PATCHing their own `timezone` to a non-IANA string raised 22023 and closed the feed, every thread, the Discover module and profile shoutouts, for every user**, silently (the app logs and renders empty). Before the reshape it broke one optional discovery filter. A CHECK cannot express it (`pg_timezone_names` is a view and CHECKs take no subqueries), so a BEFORE trigger REFUSES an unrecognised zone rather than coercing it — silently rewriting a provider's timezone would move their published hours without telling them. |
| `public.enforce_community_post_integrity` (counters) / `public.community_posts_visible` (block-on-tag) | `20261092000000` | **`20261095000000_a_counter_may_only_be_the_truth.sql`** / **`20261094000000_a_view_may_not_borrow_a_function_grant.sql`** | **Two corrections to the corrections, each a general lesson.** (a) `20261092000000` stopped an author forging their own `like_count` by carrying it over from `old` — and **broke the counter triggers with it**: `update_community_like_count` is SECURITY DEFINER and runs as `postgres`, but `auth.role()` is a JWT CLAIM, so the liker's claim is still `authenticated` and the counter's own UPDATE took the ordinary branch. The counters went back to never moving, which is the defect `20261089000000` existed to fix. **A carve-out keyed on WHO is writing could never work here — the legitimate writer and the forger present identically.** So the counter is checked the way PD-094 checks a rating: a changed count must EQUAL the recomputed count. No marker to forge, no role to impersonate; a correct value is accepted from anyone and an invented one from no one. `is_active` stays pinned outright — it has no true value to compare against. (b) The same migration added the block-on-tagged-provider filter by calling `contact_blocked_provider()` INSIDE the definer view. **A view re-targets RELATION permission checks at its owner and NOT function execute checks** — those are still evaluated against `current_user` — and that function is revoked from `anon`, so every affected read returned `42501` instead of rows. The predicate is now inlined, matching the author-side filter four lines above it. Same class as `20261044000000`'s computed column: a privilege model right for the owner and wrong for the caller, checked against the caller. |
| `public.community_posts.is_active` / `public.community_replies.is_active` (new) / `public.community_moderation_actions` (new) / `public.operator_set_community_visibility` (new) | `20260829000000_canonical_live_baseline.sql` (the column) | **`20261096000000_a_report_needs_an_outcome.sql`** | **A REPORT NOW HAS AN OUTCOME (PD-099).** `is_active` had no writer anywhere and `20261088000000` documented it as RESERVED AND UNUSED precisely so nobody would wire a button to a column that fails silently. It is now written by exactly one path. **Hiding is not deletion**: the row, the report, the case and every prior decision survive it, and the same call restores it — a moderation decision has to be reviewable, reversible and attributable, and a deleted row is none of those. **Do not turn this into a delete.** TWO conditions are required together and neither is sufficient: `is_operator()` (so no client, provider or deapproved provider can reach it) AND a session marker set only inside the RPC (so an operator cannot PATCH the column directly and leave no audit row). A marker alone is forgeable by anyone — `set_config` on a custom GUC is available to every caller — and `is_operator()` alone permits the unaudited shortcut. Refused OUT LOUD rather than silently pinned, because a privileged action that quietly does nothing is how this column became a trap. Note the two refusal MECHANISMS differ and the tests say which: for a non-author the owner UPDATE policy FILTERS the statement (zero rows, no error); for the author the trigger RAISES. The moderation log's append-only guard carves out service_role and no-claims sessions FIRST — `actor_user_id` is ON DELETE SET NULL and a set-null is an UPDATE, so without it deleting anyone who had ever moderated would fail outright, which is OQ-077 defect (2) in a new place. |
| `public.reports.reported_content_kind` / `reported_content_id` (new) / `public.operator_case_events.action` CHECK / `public.operator_community_content` (new) | `20260829000000` / `20261050000000` | **`20261097000000_a_report_must_point_at_the_content.sql`** + **`20261098000000_a_new_column_is_not_a_granted_column.sql`** | The Community post id used to travel inside `reports.notes` as free text, which was fine while nothing could act on it. Now that hiding is real, the operator screen has to RESOLVE the reference, and parsing a sentence to decide what to moderate works until someone edits the copy. Validated on intake: the content must exist, **its author must be the person the report names** (otherwise an operator acting on the case would hide the wrong thing with a straight face), and you cannot report your own content — the author already has a delete. The case-event vocabulary gained `content_hidden` / `content_restored`: filing a moderation decision as `noted` would be a state change disguised as a comment, and the event log exists so that no state change is untraceable. **`20261098000000` is the correction that had to follow**: `20261052000000` replaced this table's INSERT with a NAMED COLUMN grant, so adding two columns without adding them to the grant made EVERY report write fail with 42501 — PostgreSQL refuses the whole INSERT, not just the new columns. Same shape as the `is_mobile` defect the provider column guard was written for. **A new column is not a granted column.** |
| `public.community_moderation_actions` FK actions / `public.community_posts_read` / `public.operator_set_community_visibility` | `20261096000000` | **`20261099000000_the_record_outlives_the_content.sql`** | **FOUR CORRECTIONS FROM THE SECURITY REVIEW OF THE MODERATION WORK, one of them gating.** (a) `post_id`/`reply_id` were ON DELETE CASCADE into an APPEND-ONLY table. A referential cascade is a real DELETE that **runs as whoever issued it** — an ordinary author — so the guard saw `authenticated`, matched no carve-out and raised: **moderated content became undeletable by its own author**, with `23514` rendered as "that isn't something this account can post". **Third time in this branch that a referential action turned out to fire a rule** (`20261090000000`, `20261092000000` were the first two). Carving the cascade out would have fixed the delete and destroyed the audit trail with the content; `ON DELETE SET NULL` does both jobs, matching `actor_user_id` — **the record outlives its subject**, which is what an audit record is for. The guard admits exactly one UPDATE shape: a pointer dropped to null with every other column untouched. (b) **Hiding became a READ boundary.** The base read was `using (true)`, so any signed-in account could read every hidden post with one REST call — PD-090/PD-100 accept that diff for BLOCK inference and did not weigh a safety take-down. Now `is_active or auth.uid() = user_id or is_operator()`: the author (who is told and must be able to delete it) and an operator (who must review it), nobody else. (c) **An operator may not moderate their own matter** — their own content, a recommendation of a business they own, or a report they filed. PD-068 applied to a new surface, the rule PD-064 enforces twice for barter. (d) The session marker now **names the target row** (as `app.barter_release` and the other three markers here do) rather than a bare `'on'`, and the reply branch pins `id`. Also revoked the table-level UPDATE on `community_replies` from `authenticated`: it had no UPDATE policy, so the grant was one added policy away from being a live hole. |
| `provider_services` / `provider_availability` / `provider_blocked_dates` / `provider_policies` / `provider_booking_preferences` / `provider_profile_views` / `bookings_service_id_fkey` / `adel_provider_content` | `20261111000000`, `20260829000000_canonical_live_baseline.sql` (:1994) | **`20261120000000`**, **`20261123000000`** | **THE SAME DEFECT, ONE TABLE FURTHER OUT.** `20261111000000` narrowed the five tables I thought of. A departing provider's **entire service menu** with prices and deposit terms, their **working hours**, their **cancellation and deposit policies**, and their **blocked dates including the free-text reason** — a field people fill in with "maternity leave" — stayed publicly readable to `anon`. And none of them was ever DELETED: every one cascades from `public.providers`, and `adel_profile_account` deliberately KEEPS that row (emptied) so the contract chain survives, **so the cascade never fires and no step touched them.** A completed erasure left the `former_…` shell carrying a live, publicly readable business — a set of prices and hours that is a fingerprint, so one archived copy of the anon-readable directory re-identifies the shell and undoes the anonymisation of everything else. `bookings.service_id` had **NO ACTION** to `provider_services`, which would have made the delete fail with 23503 for any provider who had ever been booked — the same cascade class as everything else in this workstream, in the opposite direction; `SET NULL` costs nothing because `bookings.service_name` is the NOT NULL snapshot. **And then the catalogue-driven assertion earned its keep on its first run**, naming three tables nobody had mentioned: `provider_booking_preferences` (deleted — settings of a business that no longer exists), `provider_profile_views` (deleted, AND its `viewer_user_id` severed for the subject, which `adel_profile_account` was already doing for two sibling analytics tables and did not know about this one), and `provider_metrics_daily` (**KEPT** — integer counts per date with no identity of any kind, so there is nothing in it to erase; a decision, recorded as one). |
| `likes_public_read` / `"public read follows"` / `public.provider_follower_count` (new) / `app/providers/[id].tsx` | `20260829000000_canonical_live_baseline.sql` (:2598, :2783) | **`20261120000000`** | **THE JUSTIFICATION FOR A GRANT WAS FACTUALLY WRONG.** `20261111000000` kept `account_unavailable(uuid)` granted to client roles — necessary, because an RLS policy is evaluated as the caller — and argued the enumeration was closed "because the listing no longer names them". Meanwhile `post_likes` and `provider_follows` were `USING (true)` with an `anon` grant, publishing a **real `auth.users` id for every like and every follow on the platform**: an unauthenticated caller reads `post_likes?select=user_id` and then asks the oracle once per id. Neither table needs to publish anybody's identity. `post_saves` has had the right policy since the baseline, and the app reads likes exactly the same way (`.eq('user_id', user.id)`) because the visible count comes from `posts.like_count`. Follows needed one more step: the profile screen wanted a live follower COUNT over other people's rows, which is a legitimate need for a NUMBER and never for the identities — so it became `provider_follower_count()`, which also returns 0 for a hidden provider so it cannot become a second way to learn somebody is leaving. **A wrong reason for a grant is worse than a wrong grant**, because it stops the next person looking. |
| `public.account_deletion_holds` / `release_account_deletion_hold` (new) / `sweep_account_deletions` / `overdue_account_deletion_work` | `20261114000000` | **`20261120000000`**, corrected by **`20261121000000`** and **`20261122000000`** | **THREE WAYS A HOLD WAS A ONE-WAY DOOR, AND A HOLD THAT DID NOT HOLD.** (a) The operations note promised a hold "is released the same way" it is placed and **nothing in the schema ever wrote `released_at`** — releasing one was raw SQL and it resumed nothing. (b) A `held` step was not in the retry set and the request had already reported `completed`, so `finalize_account_deletion` returned at its first guard forever after: held work was done by nothing. (c) **The purge loop never consulted holds at all** — the class mapping lives in `run_account_deletion_step` and the two purges are run directly by the sweep, so a legal hold on `messages` would have watched the messages be deleted. A hold that does not hold is worse than no hold, because somebody is relying on it. `overdue_account_deletion_work()` also could not see the failure its own comment calls the worst one: a request past its grace date that nobody swept has twelve `pending` steps with no `due_at`, and the predicate required one. **Two corrections followed, both caught by the suite on the next run:** a table alias `r` collided with the declared record variable `r` and every sweep raised `55000: record "r" is not assigned yet`; and moving a request from `completed` back to `failed` while leaving `completed_at` set violated `account_deletion_requests_completed_check` — which is the constraint being right, because a request with work left is not completed and must not keep a completion stamp. |
| `prevent_provider_verification_self_update` | `20260829000000_canonical_live_baseline.sql` (:78-97) | **`20261120000000`** | `20261118000000` wrote the rule down — an unqualified name inside a SECURITY DEFINER function is how a caller-controlled schema hijacks a privileged query — and then fixed five NON-definer functions while leaving the one **definer** trigger on `public.providers` with no `search_path` at all. It fires on every providers UPDATE, the erasure's included. Not currently exploitable (its body touches only `auth.role()` and NEW/OLD, no unqualified relation) — this is the rule applied to the function that most obviously falls under it, with the BODY VERBATIM from the baseline including all five verification columns, per `20261119000000`'s standing warning. The suite now asserts generically that **no** definer function in `public` leaves `search_path` unpinned, which is the only form that finds number two. |
| `providers_public_read` / `posts_public_read` / `comments_public_read` / `community_posts_read` / `community_replies_read` / `public.account_unavailable` (new) / `refuse_provider_write_when_account_inactive` (new) | `20261102000000` | **`20261111000000`**, **`20261117000000`** | **PD-104. THE GUARANTEE WAS IN THE VIEW, NOT IN THE DATA.** "Your profile is hidden" was enforced in six `_visible` views while `providers_public_read` stayed `USING (true)` and `comments_public_read` with it — one REST call returned every departing provider's name, bio, location and photos, and **the app's own profile screen read exactly that table**. The base tables now carry the rule, with three carve-outs that are load-bearing rather than concessions: the owner, an operator, and a counterparty who already has a booking or conversation (sixteen screens read `providers` directly to resolve a live transaction, and PD-102 forbids stranding one). Three further lessons: (a) an **RLS policy is evaluated as the CALLER and can borrow nothing** — a definer view re-targets relation checks but not function checks, a policy re-targets neither, so the operator clause became its own `TO authenticated` policy after `20261116000000` granted `is_operator()` to `anon` and two suites rejected it inside the hour; (b) **"pending deletion" was the wrong question** — it keyed on a column the auth delete SETS NULL, so it inverted to false at the instant of erasure and an erased identity read as live, against five tables that carry no FK to `auth.users`; `account_unavailable` reads durable `erased_accounts` instead, and answers one boolean over three causes so it cannot be read as "this person is leaving"; (c) the account-inactive refusal on `providers` tested the CALLER on every UPDATE to any row, so completing a booking — which recomputes a rating through `update public.providers` under the same JWT — gave a departing provider PT440 for **finishing their own client's work**, and the client could then never review. |
| `public.contracts.user_id` / `public.adel_accepted_contracts` | `20260829000000_canonical_live_baseline.sql` (FK at :2094) | **`20261112000000`** | **I PRESERVED THE SIGNATURE AND LEFT THE CASCADE THAT DELETES IT.** `20261103000000` made the accepted contract survive the CLIENT and I checked one direction. `contracts.user_id` was `ON DELETE CASCADE`, so erasing the **PROVIDER** deleted the contract, which cascaded into `contract_versions` and `contract_signatures` — and the version guard explicitly permits DELETE on erasure paths, so the whole chain went silently and the step SUCCEEDED. The loss belonged to every client of theirs: the exact terms they accepted, when, and for which booking. A record-class hold could not have stopped it either, because a referential cascade consults nothing. Now `SET NULL` both ways. **No `owner_subject_id` column was added, deliberately:** `contracts.provider_id` points at the providers row the engine keeps emptied rather than deleted, so the provider-side party identity is already durable — and `contracts` carries `GRANT ALL` to `anon` and `authenticated`, where a new column would have been readable the moment it existed. |
| `public.post_comments` / `public.adel_community_content` / `public.adel_provider_content` / `public.sweep_account_deletions` / `public.overdue_account_deletion_work` (new) / `enforce_erased_accounts_append_only` / `finalize_account_deletion` | `20261104000000`, `20261107000000` | **`20261113000000`**, **`20261114000000`**, **`20261115000000`** | **FOUR WAYS A COMPLETED ERASURE WAS NOT ONE.** (a) `post_comments.user_id` has **no foreign key to `auth.users`**, so nothing cascaded and no step touched it: a person's Reel comments survived under their REAL account id, `USING (true)`, readable by `anon` — the join key that would have made every other anonymisation in this workstream reversible. (b) The media queue was fed only by the provider-content step, which **returned early for an account with no providers row** — client avatars go to the same PUBLIC bucket under the same prefix, so every client-only erasure reported `completed` with the person's face still served at a non-expiring URL. (c) The purge loop took only `scheduled` and its handler wrote `failed`, which nothing ever read again: **one transient lock turned a 180-day message window into forever**, silently, while the request said `completed`. Now retried, locked with `SKIP LOCKED`, and reportable through `overdue_account_deletion_work()` — a failure mode invisible by construction needed a query, not a paragraph. (d) `erased_accounts` permitted DELETE, which would let a later run mint a SECOND pseudonym for the same person — the "one person becomes several" outcome its own guard existed to prevent. Plus `barter_version_acceptances.participant_subject_id` was added to a table-level grant and restricted **nothing** — the third occurrence of a mistake written down twice in capitals, now guarded by one generic assertion over every `%_subject_id` column instead of five by name. And the retention fallbacks `coalesce(retention_days(...), 90)` are gone: an unset window produces a `held` step somebody has to decide, because "never zero and never infinite" has to mean something in code. |
| `update_post_comment_count` / `update_post_like_count` / `update_post_save_count` / `update_community_like_count` / `update_community_reply_count` | `20260829000000_canonical_live_baseline.sql` (:229-311), `20261089000000` (:196-231) | **`20261118000000`**, corrected by **`20261119000000`** | **A TRIGGER FUNCTION INHERITS THE CALLER'S `search_path`.** Five baseline counter triggers had none set and used unqualified relation names. Every erasure step is `security definer set search_path = ''` — as every definer function here must be — so the moment a step deleted a comment, like, save or reply, the AFTER trigger died with `42P01: relation "posts" does not exist` and **failed the erasure for any account that had ever liked a post**. For a consumer app that is close to every account. No fixture had ever seeded a like, so nothing caught it until the new suite did, on its first run. **And then the fix broke something else:** I took all five bodies from the canonical baseline, but two had been redefined by `20261089000000` as `security definer`, and a `create or replace` replaces the WHOLE definition — security context included. Without it the counter's UPDATE ran as the LIKER, who is not the post's author, matched no row under RLS, and **every community like and reply count silently stopped moving** — the identical failure `20261095000000` documents from the other direction. Three assertions caught it. **RULE: never reconstruct a function body from the baseline. `grep` every definition of the name and take the LAST.** `20261066000000` is this same lesson for views. |
| `public.retention_policy` (new) / `public.account_deletion_requests` + `_steps` + `_holds` / `public.erased_accounts` (new) / `public.account_pending_deletion` (new) | — | **`20261100000000`** … **`20261102000000`** | **ACCOUNT ERASURE (PD-102).** Retention windows are ROWS, not constants: the engine, the app and the support docs read the same fact, and the two awaiting counsel are **NULL and flagged** rather than guessed — `retention_days()` RAISES on an unknown key so a typo cannot read as "no retention". The deletion is a JOB WITH A MEMORY: per-class steps make a retry resumable rather than a restart, and `completed` is RECOMPUTED from the step rows so a failed or unrun step keeps a request out of it by existing. Holds are **per class**, deliberately — a global hold would turn one open report into a reason to keep somebody's credentials, which the policy forbids. **Deactivation is DERIVED from the open request, not stored**: it cannot drift, cancelling the request IS the restoration with nothing to repair, and `providers.is_approved` is left alone so an operator's moderation decision and a user's own choice stay distinguishable. `erased_accounts` holds the only surviving link: a restricted `subject_id` (the minimum party identity contracts and reports may keep) and a `pseudonym_id` — the latter because nulling an erased reviewer would merge every departed reviewer into ONE voice under PD-091/092's distinct-client rule and move the rating of every provider they had reviewed. |
| `public.contract_signatures` / `public.reports` / `public.operator_case_events` / `public.providers.user_id` / barter + booking-photo identity keys | `20260829000000_canonical_live_baseline.sql` | **`20261103000000`**, **`20261105000000`**, **`20261108000000`** | **THE CASCADES THAT WOULD HAVE DESTROYED WHAT THE POLICY RETAINS.** An `auth.users` delete cascaded through contracts, reports, reviews, bookings (via `providers`) and the whole barter chain — `proposals` → `versions` → `agreements` → `obligations`, so erasing one participant deleted **the counterparty's record of a completed, adjudicated trade**. Retained classes now hold a **restricted `*_subject_id`** with no FK, absent from every client column grant; anonymized classes take `ON DELETE SET NULL`; and `providers` is kept OWNERLESS AND EMPTIED rather than deleted, because bookings reference it and policy B keeps the operational record — with `user_id is not null` added to `providers_visible` and to the booking gate, since `account_pending_deletion(null)` is false and a shell would otherwise reappear in discovery. **OQ-077 (1)** is fixed by widening `reports_target_check` to accept the retained subject: the delete succeeds AND the safety record survives, rather than one at the expense of the other. |
| `public.enforce_operator_case_event_append_only` / `enforce_moderation_actions_append_only` / `enforce_barter_obligations_immutable` / `enforce_barter_offer_write` / `enforce_barter_interest_write` / `retain_identity_on_sever` (new) | various | **`20261106000000`**, **`20261109000000`**, **`20261110000000`** | **OQ-077 (2), AND THE GENERALISATION IT TURNED OUT TO BE.** Every append-only and immutability guard in this schema was written against a client trying to rewrite history, and **a referential `SET NULL` looks exactly like one** — so six of them refused the erasure sever. Each gained the same narrow allowance, checked IN FULL: the identity column to NULL, every other column identical, so it can forget WHO and never change WHAT; the obligation guard additionally refuses a REPOINT, and its set-difference still binds `service_role` as its own comment intends. `retain_identity_on_sever` then made retention **a property of the sever rather than of step ordering**: the engine wrote the restricted id before nulling the live one, so the supported path worked while a RAW `delete from auth.users` still failed — dashboard, admin API, ops script. A guarantee that depends on calling things in the right order is a convention with a test. |
| `public.pending_media_deletions` (new) / `adel_*` step functions / `public.finalize_account_deletion` | `20261104000000` | **`20261107000000`** | **TWO INFRASTRUCTURE TRUTHS, HANDLED RATHER THAN ASSUMED AWAY.** (a) **SQL cannot delete a storage object** — Supabase answers `42501: Direct deletion from storage tables is not allowed`. Objects are queued and a final `media_purge` step RAISES while any remain unconfirmed, so **no erasure reports success with the bytes still in the bucket**; an operator drains the queue through the Storage API and calls `confirm_media_deleted`. There is no scheduler, and that is documented rather than implied away. (b) **A column-level revoke does not override a table-level grant**, so `20261103000000`'s revoke of the restricted identity columns had NO EFFECT and they were readable by any signed-in user — the exact opposite of "restricted". Fixed with the shape `20261052000000` already used for `reports`: revoke the table-level SELECT, re-grant the ordinary columns by name. A revoke that looks right and does nothing is the more dangerous kind of wrong. |
| `public.erasure_relationship_pseudonyms` (new) / `erasure_relationship_pseudonym` (new) / `erasure_pseudonyms_for` (new) / `record_erased_account` (new) / `public.erased_accounts.pseudonym_id` (DROPPED) / `adel_bookings` / `adel_reviews` / `adel_messages_sever` / `adel_messages_purge` / `finalize_account_deletion` | `20261101000000`, `20261104000000` | **`20261124000000`**, corrected by **`20261127000000`** | **PD-105 / OQ-085. ONE PSEUDONYM ACROSS A WHOLE LIFE IS DE-NAMING, NOT ANONYMISATION.** The engine wrote a single id into a departing person's bookings, conversations, messages and both review tables. `provider_reviews` is readable by `anon` for revealed reviews, so a provider holding ONE booking row could read the id off it and walk the public review list to learn **every other provider that person used, when, and what they wrote** — a complete service history, recovered from an account that had been told it was erased. The pseudonym is now allocated per **(subject, scope)**: `provider` for bookings and both review tables, which is the minimum width PD-091/PD-092 need (the public rating averages one review per DISTINCT client, so merging erased reviewers moves every affected provider's rating), and `conversation` for threads, which is narrower still. Bookings and reviews share the provider-scoped id deliberately — a review already names its booking, so nothing new is disclosed. **The person-wide column is DROPPED rather than left unused**, because this repo's history is a list of dormant things that came back (`posts.is_active`, `20261088000000`), and the append-only guard was redefined FIRST since a plpgsql body naming a dropped column fails at runtime on every insert. The reverse index is PERMANENT and that is a requirement, not an oversight: the two scheduled purges locate their rows through it up to 180 days later. It is revoked from every client role — **and an operator is a client role**, which is what makes "not resolvable by ordinary operator lookup" true; `service_role` can read it, the same trust class as `erased_accounts` under PD-101, and that is stated rather than glossed. **`20261127000000` is the correction the suite found on its first run:** the guard refused a re-key and then `return coalesce(new, old)`, which permits a DELETE — the identical defect `20261114000000` had to fix on `erased_accounts`, written again two files later in the same branch. Deleting the map would make a purge retried from `failed` find nothing, delete nothing and report success: a 180-day window silently becoming forever. |
| `refuse_row_write_when_account_inactive` (new) / `enforce_booking_submit_when_account_inactive` (new) / `refuse_barter_offer_edit_when_account_inactive` (new) / `refuse_barter_interest_edit_when_account_inactive` (new) / `refuse_conversation_accept_when_account_inactive` (new) / 21 new triggers / 8 `storage.objects` write policies | `20261102000000`, `20261111000000`, `20260829030000`, `20261033000000`, `20260829060000`, `20261072000000` | **`20261125000000`**, corrected by **`20261127000000`** | **PD-106 / OQ-086. THE GATE COVERED THE PATHS SOMEBODY THOUGHT OF.** `20261102000000` put a BEFORE INSERT refusal on fourteen tables. A full enumeration of the `authenticated` write surface found the rule missing from thirty-odd more places, and **the worst was not a missing table but a missing VERB**: the app sends a booking by UPDATING an existing draft's `submitted_at` (`lib/bookingDraft.ts:308`), so a deactivated account holding a draft could send a real new booking request with no refusal at all. The block workstream had the identical hole and fixed it in `20261058000000`; **the same shape was missed twice**, which is why the new gate is named as that function's twin. Also closed: every barter negotiation RPC — `create_barter_proposal`, `submit_barter_counter`, `accept_barter_version` and above all `finalize_barter_agreement`, which creates a BINDING AGREEMENT with new obligations — none of which checked anything. They are `SECURITY DEFINER`, but **a definer function does not change `auth.uid()` or `auth.role()`**, so a trigger on the table it writes sees the real caller; gating the five tables is both sufficient and far safer than `create or replace`-ing five large bodies, which is how this repo has lost a rule three times. And storage: all five buckets accepted uploads from a deactivated account, including the two PUBLIC ones. **THE COUNTER TRAP WAS AVOIDED ON PURPOSE.** `update_post_like_count` UPDATEs `posts.like_count` from an AFTER trigger running under the LIKER's JWT, so a blanket UPDATE refusal would have stopped a departing person un-liking their own post — SEC-DATA-011 in a new place, where a departing provider got PT440 for completing their own client's booking. The gate refuses an UPDATE only when the row is the caller's OWN and a NON-DERIVED column moves, with the derived list passed per trigger. **`20261127000000` drops one trigger this file added:** `barter_obligations` is written only by `finalize_barter_agreement`, in the same transaction as the gated `barter_agreements` row, so it added no coverage — and three suites pin "no new trigger on `barter_obligations`" for a reason that has nothing to do with erasure (an obligation's state must never be moved by a clock). Spending that signal on redundant defence is a bad trade. |
| `public.adel_contract_artifacts` (new) / `account_deletion_step_keys` / `run_account_deletion_step` / `finalize_account_deletion` | `20261104000000`, `20261107000000`, `20261112000000` | **`20261126000000`** | **PD-107 / OQ-087. THE POLICY RETAINED THE EVIDENCE AND EVERYTHING SITTING BESIDE IT.** `20261112000000` kept the contract chain alive through an erasure — correct, because deleting it destroys every CLIENT's record of terms they accepted — but kept ALL of it, and touched neither contract bucket. `contract-pdfs` and `contract-signatures` were the only storage prefixes no step looked at, so a real account id survived in an object path indefinitely. Retention is now the **canonical accepted artifact**: the version a signature bound to, its PDF, the timestamp, the minimum party identity. A contract nobody ever signed is deleted; a version no signature points at is deleted; every object under the subject's prefix that no surviving row names is queued for the Storage API. **A contract referenced only by a PENDING or DECLINED signature is deliberately NOT deleted** — that row is the counterparty's record, not this person's to remove, and the FK forbids it anyway. **The signature-artifact condition resolves to NOTHING, as a fact rather than a judgement:** `signature_url` is always NULL, the canvas was removed, `app/book/payment.tsx:189` writes an explicit null and `lib/storage.ts:7` records the bucket as unwritten — so the ruling's *"only if the implementation relies on it"* is not met and the suite asserts no signature image exists anywhere, which is what makes reintroducing the canvas trip a test rather than ship an artifact the retention decision never considered. The step runs BEFORE `accepted_contracts` because that step severs `contracts.user_id`, after which the drafts cannot be identified as this person's; it answers to the `accepted_contracts` HOLD class, so a legal hold stops the narrowing too. `finalize_account_deletion` now SEEDS missing step rows before the loop — steps are written at REQUEST time, so adding a class left in-flight requests without the row and `run_account_deletion_step` raises **outside** its own exception block, aborting the whole finalisation rather than failing one step. |
| `scripts/account-deletion-worker.mjs` (new) | — | **not a migration** | **THE EXECUTION HALF, AND THE HALF THAT WAS STILL MISSING AT THIS POINT (OQ-088). Superseded 2026-09-13 by `20261130000000` / PD-108 — the scheduler section below — which closed OQ-088 and made this worker the FALLBACK. The rest of this cell is the state as of PR #83 and is kept for that record.** `sweep_account_deletions()`, `pending_media_deletions`, the Storage API deletes, `confirm_media_deleted(...)` and `overdue_account_deletion_work()` all existed and **nothing invoked any of them**. The worker does, in one bounded idempotent command, in the directory that already holds `db-security-test.mjs` and `seed-nonprod.mjs`, on the same tooling env and the same hard production-ref guard — **no new infrastructure and no new platform primitive**. What it is NOT is a scheduler: nothing runs it on a clock, and every way to supply one is a platform decision (`pg_cron`+`pg_net`, which two suites currently ASSERT is not installed; Supabase scheduled functions, needing a project config this repo has never held; scheduled CI, putting production service-role credentials in the CI provider; or an external host). Returned as a **PRE-EXTERNAL-BETA BLOCKER** rather than invented. |
...
**Validation after the final apply** (non-production `wcoyjeklscuqsumpjpfo`; production untouched):
B5B **1516/1516** with zero residue, concurrency **198/198** across two consecutive runs, Jest
**915/915**, typecheck clean, lint 0 errors, and `supabase migration list` showing local == remote
with no drift. Recorded here because this section previously carried an apply DATE and no figures,
so the most recent numbers in the whole ledger were Correction 2's — which made it impossible to
tell, from the ledger alone, whether either later block had ever been validated at all.
...
the same branch and all from the security review of the first commit. **That branch
merged to `main` as `719d8f9` (PR #84) on 2026-09-13**, so all four files are on
`main` and the chain is **172** files, `20260829000000` … `20261133000000`:
...
**Re-validated on the merged tree** (2026-09-13, after the final security-review
commit added the schema gate and its test): typecheck clean, `lint:ci` **0 errors /
209 warnings** against the frozen 210 baseline, Jest **1030/1030** across **53**
suites — the earlier **1003** predates those tests. **B5B and the concurrency
harness were NOT re-run** on the merged tree and their figures above stand as last
measured on the branch; CI's `db-security` job is the standing DB gate and passed
on the merged head.
