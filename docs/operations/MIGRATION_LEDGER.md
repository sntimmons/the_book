...
|---|---|---|---|
| `public.enforce_barter_interest_write` | `20260906000000_barter_integrity_slice1.sql` | **`20260909000000_barter_interest_release.sql`** | Adds the `accepted -> released` transition and the release-column allow-list, gated on a transaction-local marker set only by `release_barter_interest`. The pre-existing owner-only `pending -> accepted\|declined` rule and the INSERT clamps are carried through unchanged. |
