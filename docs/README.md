...
| [product/FUTURE_PRODUCT_IDEAS.md](product/FUTURE_PRODUCT_IDEAS.md) | Product / exploration | **Exploratory - NOT authoritative** - concepts being explored and the reasoning behind them: the transaction-protection exploration (SS 2) and the barter matching/liquidity exploration (SS 3). Nothing in it is decided, built, or promised; it is the deliberate opposite of `PRODUCT_DECISIONS.md`. |
...
| [design/THIRD_VISUAL_SYSTEM.md](design/THIRD_VISUAL_SYSTEM.md) | Design | **Authoritative** - the durable cross-app visual principles: the semantic colour roles, the Light/Dark/System model and why it is a mixed surface model rather than an inversion, restraint in containers and spacing, hierarchy and action emphasis, and the rule that social content may support discovery but never imply provider quality or rank. Names the guard behind each rule that has one. The founder-protected Welcome experience is explicitly excluded. |
| [design/MESSAGES.md](design/MESSAGES.md) | Design | **Authoritative for that surface; engineering-complete for the buildout phase, final visual consistency approval deferred to the whole-app visual pass** - the "Working Letter" direction built in Sessions 7B and 7C across all THREE Messages screens - inbox, first-contact composer and thread: hairline inbox rows, the All -> Conversations label correction and why it was untrue, empty states that instruct nothing, message slabs rather than chat bubbles, derived day separators, the context band and why safety notices stay out of it, and the list of capabilities this surface does not have and must not grow by imitation. |
| [design/REELS.md](design/REELS.md) | Design | **Authoritative for that surface; engineering-complete for the buildout phase, final visual consistency approval deferred to the whole-app visual pass** - the "Held Light" art direction built in Session 6B: the immersive full-bleed point of view, the single identity block, tap-to-pause with the paused state stated in a word, the removal of the decorative centre glyph and of every engagement count, the "View & book" primary action, and the provider-only shortcut into the existing Posts & Reels uploader. Screen-specific by design - it names what is NOT a cross-app rule. |
| [design/DISCOVER.md](design/DISCOVER.md) | Design | **Authoritative** - the approved design of Discover as built and founder visually approved (2026-09-15): marketplace-first hierarchy, header and area, search, the four lanes and their visible rules, From people you follow, the uniform Everyone on Third grid, See the work, Community as secondary, and the spacing rhythm. Operational lane rules live in operations/DISCOVERY_OPERATIONS.md. |
...
| [operations/MEDIA_UPLOAD.md](operations/MEDIA_UPLOAD.md) | Operations / Engineering | **Authoritative** - the shared upload contract in `lib/storage.ts`: what it returns, what it refuses, the native dependency and the rebuild it implies, and the invariant that every product-created video post must carry a usable still for surfaces that cannot play video. |
...
| [operations/ACCOUNT_ERASURE_OPERATIONS.md](operations/ACCOUNT_ERASURE_OPERATIONS.md) | Operations / Support | **Authoritative** - the support note for account deletion and retention: the user's flow, restoration, what is deleted / anonymized / retained, what a deactivated account may still do, holds, the automatic execution path and its manual fallback, and the limits beside each. Named as authoritative in the prose below since it was written; it had no index row until 2026-09-12, so a reader following this table could not find it. |
